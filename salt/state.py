            ...
            for state, run in body.items():
                ...
                if names:
                    ...
                    for fun in funcs:
                        live["fun"] = fun
                        chunks.append(live)
        return self.order_chunks(chunks)
...
class State:
    ...
    def verify_data(self, data):
        ...
        if data["name"] and not isinstance(data["name"], str):
            errors.append(
                f'''ID '{data["name"]}' {f"""in SLS '{data["__sls__"]}' """ if "__sls__" in data else ""}is not formed as a string, but is a {type(data["name"]).__name__}'''
            )
        if errors:
            return errors
